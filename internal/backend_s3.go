...
func createRequest(host string, method string, filePath string, body io.ReadSeeker, storageClass string) *http.Request {
	...
	// Generate hash for payload. If it's put it's the body else empty
	sha256hashedPayload := ""
	md5hashedPayload := ""
	contentLength := ""
	if method == "PUT" {
		...
		hasher := sha256.New()
		hasher.Write([]byte(""))
		body = strings.NewReader("")                              // does this work?
		sha256hashedPayload = hex.EncodeToString(hasher.Sum(nil)) // should be e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
	}

	// combine host and filepath to get full url string
	req, err := http.NewRequest(method, "https://"+host+filePath, body) //prev payload
	if err != nil {
...
func hashAndLengthReadSeeker(rs io.ReadSeeker) (string, string, string, error) {
	// Create a new SHA256 hash
	sha256Hash := sha256.New()
	md5Hash := md5.New()
	// Save the current position of the ReadSeeker
	startPos, err := rs.Seek(0, io.SeekCurrent)
	if err != nil {
		return "0", "", "", err
	}
	contentLength, err := io.Copy(io.MultiWriter(md5Hash, sha256Hash), rs)
	if err != nil {
		return "0", "", "", err
	}
	md5 := md5Hash.Sum(nil)
...
func (s *S3Backend) PutBlob(param *PutBlobInput) (*PutBlobOutput, error) {
	...
	put := &s3.PutObjectInput{
		Bucket:       &s.bucket,
		Key:          &param.Key,
		Metadata:     metadataToLower(param.Metadata), // this didnt print out anything really.
		Body:         param.Body,
		StorageClass: &storageClass,     // doesnt seem to print out
		ContentType:  param.ContentType, //empty for a .txt file
	}
	s3Log.Debugf("PutObjectInput: %v", param)

	// None of the following ifs are used
	if s.config.UseSSE {
